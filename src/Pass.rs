...
extern {
  ...
  pub fn mlirPassManagerEnableIRPrinting(passManager: MlirPassManager, printBeforeAll: u8, printAfterAll: u8, printModuleScope: u8, printAfterOnlyOnChange: u8, printAfterOnlyOnFailure: u8) -> ();
...
impl FFIVoid_ {
  pub unsafe fn mlirPassManagerEnableIRPrinting<T0_, T1_, T2_, T3_, T4_, T5_>(passManager_:  T0_, printBeforeAll_:  T1_, printAfterAll_:  T2_, printModuleScope_:  T3_, printAfterOnlyOnChange_:  T4_, printAfterOnlyOnFailure_:  T5_)
  where
     T0_: Into<MlirPassManager>,  T1_: Into<u8>,  T2_: Into<u8>,  T3_: Into<u8>,  T4_: Into<u8>,  T5_: Into<u8>
  {
    unsafe {
      crate::Pass::mlirPassManagerEnableIRPrinting(Into::<MlirPassManager>::into(passManager_), Into::<u8>::into(printBeforeAll_), Into::<u8>::into(printAfterAll_), Into::<u8>::into(printModuleScope_), Into::<u8>::into(printAfterOnlyOnChange_), Into::<u8>::into(printAfterOnlyOnFailure_))
    }
  }
}
